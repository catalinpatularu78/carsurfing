...
public class RideRestController {
	...
	ResponseEntity<?> get(@RequestHeader("Authorization") String header) {
		...
		ResponseEntity<Boolean> responseObj = restTemplate.exchange(userServiceURL + "generalauth", HttpMethod.GET,
				entity, Boolean.class);
		Boolean pass = responseObj.getBody();
		if (pass) {
			return ResponseEntity.ok().body(rideRepo.findAll());
		} else {
			return ResponseEntity.badRequest().body(new String("Error: User not logged in!"));
		}
	...
	public ResponseEntity<?> getRideById(@PathVariable(value = "id") Integer rideId,
		...
		try {
			...
			if (pass) {
				...
				return ResponseEntity.badRequest().body(new String("Error: User not found!"));
			}
		} catch (Exception e) {
	...
	Response addRide(@RequestBody Ride ride, @RequestHeader("Authorization") String header) {
		...
		if (pass) {
			try {
				...
				responseMessage = "Ride from location " + ride.getFromLocation() + " to destination "
						+ ride.getToLocation() + " added.";
				return new Response(responseCode, responseMessage);

			} catch (Exception exc) {
				responseCode = ResponseCode.FAILED;
				responseMessage = "Ride from location " + ride.getFromLocation() + " to destination "
						+ ride.getToLocation() + " cannot be added.";
	...
	public ResponseEntity<?> findRidePerRouteAndDate(@RequestBody Map<String, String> rideRequest,
			@RequestHeader("Authorization") String header) {
			...
			try {
				...
				if (existingRide.isPresent()) {
					...
					if (existingBooking.isPresent()) {
						return new ErrorResponse(ResponseCode.FAILED, "Ride cannot be booked.",
	...
	@GetMapping("/rides/users/{userId}")
	public ResponseEntity<?> getRideByUserId(@PathVariable(value = "userId") Integer userId,
			@RequestHeader("Authorization") String header) {
		try {
			HttpHeaders headers = new HttpHeaders();
	...
	// returns rides for any user: driver or passenger
	// if a user is both(driver in one ride and a passenger in a different ride),
	// both rides are returned
	private List<Ride> getRidesForUser(Integer userId) {
		List<Ride> rides = rideRepo.findByDriverId(userId);
		List<Booking> bookings = bookingRepo.findByPassengerId(userId);
		if (!bookings.isEmpty()) {
			for (Booking booking : bookings) {
				Optional<Ride> ride = rideRepo.findById(booking.getRideId());
				if (ride.isPresent()) {
					rides.add(ride.get());
				}
			}
